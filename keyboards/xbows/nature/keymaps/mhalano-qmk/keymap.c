...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  [0] = LAYOUT(
		...
		TD(TD_CAPS), KC_A,    KC_S,  KC_D,   KC_F,   KC_G,    KC_BSPC, KC_H,  KC_J,   KC_K,    KC_L,   KC_SCLN, KC_QUOT, KC_ENT,  KC_PGDN,
...
// Must be this function and not rgb_matrix_indicators_user() because host_keyboard_led_state() is not available in the latter.
...
void on_tap(tap_dance_state_t *state,  void *user_data) {
    switch (state->count) {
        case 1:
            if (host_keyboard_led_state().caps_lock) {
                tap_code16(KC_CAPS);
...
tap_dance_action_t tap_dance_actions[] = {
    [TD_CAPS] = ACTION_TAP_DANCE_FN(on_tap)
};
